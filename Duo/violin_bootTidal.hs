...
:{
let oscmap = [
              ((superdirtTarget {oLatency = 0.15, oPort = 57120}), [superdirtShape]) -- SuperCollider port
              ,
              ((superdirtTarget {oLatency = 0.15, oPort = 57121}), [superdirtShape]) -- p5.js port
             ]
:}
...
let only = (hush >>)
    ...
    hardpan = pan (choose[0, 1])
    softrand = (((range 0 1 $ rand) - 0.5) * 0.1) + 1
    densmask st en p = scramble (round <$> (range st en $ sine)) $ fast (range st en $ sine) $ s p
